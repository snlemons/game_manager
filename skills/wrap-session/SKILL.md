...
Apply the **Locate-or-ask** shape from `../../references/campaign-locate.md` — that reference is the canonical spec for the four-marker check (`CLAUDE.md`, `.claude/rules/sessions.md`, `.claude/rules/adventures.md`, `campaign.md`) and the cwd-then-ask-the-GM routing. `/wrap-session`'s skill-specific orchestration on top of the shared shape:
...
- **PC-actor narrative framing discriminates new PCs from new NPCs at extraction time** per `../../references/reference-note-extraction.md` "PC-actor narrative framing as wrap-time discriminator." A named character narrated as the actor of party-shaped actions (e.g., *"Theron drew his sword and charged"*) is a PC signal; subject-of-prep framing (*"the party met Maren at the docks"*) is the NPC default. Confident PC-actor framings stage at `pcs/<slug>.md` directly without a Step 3 ASK; ambiguous framings default to NPC and surface a PC-or-NPC ASK at Step 3. The GM can also correct in the Step 4 proposed-wrap review by moving the staged file between `pcs/` and `npcs/`.
...
- **PC vs NPC (safety net)** — per [ADR-0018](../../docs/adr/0018-pc-roster-as-survey-deliverable.md), refined by the wrap-time PC-actor discriminator in `../../references/reference-note-extraction.md` "PC-actor narrative framing as wrap-time discriminator": a named character in `notes.md` whose name doesn't match any existing `pcs/<slug>.md` filename or `aliases:` entry **and** doesn't match any existing `npcs/<slug>.md` either (so the Pass 2 dedup pass would otherwise propose a CREATE under `npcs/`), and whose narrative framing is **ambiguous** between PC-actor (subject-of-party-action, party-pronoun grouping, player-attribution shorthand) and NPC (subject-of-prep, role-tag, stance-toward-party). The ASK shape per the discriminator reference: *"`Marisa` appears in this session's notes — narrated as an actor doing PC-actor things in some sentences and as a subject in others. PC or NPC?"* On *"PC"*, drop the proposed `npcs/marisa.md` CREATE and stage a `pcs/marisa.md` stub instead, same shape as `/ingest` Phase 2's stubs (`kind: pc` frontmatter, optional `aliases:` from any nickname the GM provides, optional one-line body if the notes give a clear role one-liner — otherwise H1-only). Any Beat `linked_pcs:` or Secret `belongs_to:` references the agent drafted using the rejected NPC slug are rewritten to point at the new PC slug before staging. `/wrap-session` is single-session, so this confirmation does not become a carried-forward lesson — it applies only to this wrap; future sessions re-confirm if needed (though the just-created `pcs/marisa.md` will route subsequent `Marisa` mentions to PC via the standard PC-vs-NPC discriminator). On *"NPC"*, the proposed `npcs/marisa.md` CREATE stands. **Pass 2 routes confident PC-actor framings directly to `pcs/` without firing this ASK** — the question is for the ambiguous cases the wrap-time discriminator couldn't resolve. The GM can also correct confident-but-wrong routing in the Step 4 proposed-wrap review by moving the staged file between `.ttrpg-staging/wrap/npcs/<slug>.md` and `.ttrpg-staging/wrap/pcs/<slug>.md`; Step 5 promotes from whichever path the GM left the file in. Less load-bearing than the `/ingest` Phase 3 safety net because `/wrap-session` runs against an established campaign where the roster usually exists — but worth including for the case where a brand-new PC first appears in a session log.
