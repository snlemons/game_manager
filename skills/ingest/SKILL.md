...
For each surviving PC roster entry, draft a stub file per the shape below and stage it at `.ttrpg-staging/pcs/<slug>.md` per `../../references/staging-pattern.md` Section 2 (CREATE entries: Write the full proposed content):
...
- `aliases:` carries the `— alias: <name>` entries from the staged line. If there are no aliases, omit the key entirely (per `../../references/frontmatter-schemas.md` Reference-note default — absent reads as `[]`).
...
2. Doesn't match any existing `pcs/<slug>.md` filename or `aliases:` entry in the campaign repo (apply the same matching rule used for Reference-note dedup — `../../references/dedup-matching.md`), **and**
...
| Reference note (UPDATE) | `.ttrpg-staging/doc-<N>/<kind>/<slug>.md` — stage per `../../references/staging-pattern.md` Section 2 (cp the live file, Edit to apply the proposed change so the IDE diff shows the delta) |
...
5. **Run bidi maintenance after each Secret write.** Per `../../references/bidi-link-maintenance.md`'s `apply_belongs_to`: for each container in the Secret's `belongs_to:`, confirm the container's file body now contains the `## Secrets` section wiki-link. The "back-reference present?" check accepts **either** canonical slug-path form (`[[secrets/<slug>]]`) **or** canonical-title display-name form (`[[<title>]]` where `<title>` is the Secret's H1) — v0.1/v0.2-era campaigns may carry display-name back-references that pre-date this Phase 3 invocation; those satisfy the symmetry and the writer does not duplicate them. New back-references the writer authors use canonical slug-path form only. If a container is missing the back-reference in both forms (the GM deleted the staged container back-reference UPDATE but the contract-violation prompt was overridden, or the container is a pre-existing file whose back-reference wasn't staged because it was already present at stage time but has since been edited), surface it to the GM as a post-write reconciliation prompt. The algorithm is idempotent — re-running on an already-linked container is a no-op (`tests/test_bidi_link.py::TestApplyBelongsTo::test_is_idempotent_on_rerun`).
