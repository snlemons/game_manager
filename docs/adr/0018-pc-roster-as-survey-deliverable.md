...
Status: superseded by [ADR-0022](./0022-pc-roster-via-explicit-classification.md) — the candidate-source layer (skim-based PC inference during the bounded skim, "Likely PC" / "Possible NPC" classification, the prose-signal heuristics) is stepped back. The broader policy this ADR established (PC roster as a Phase 2 deliverable, dual-file review batch, single-doc stripped-survey scope, scaffold-only skip scope, Phase 3 safety net, composition with #57, stub shape) stays in force; see ADR-0022 for the refined mechanism. The body below is preserved as the historical record of the v0.2 mechanism.
