...
- The template file lands under `templates/.claude/rules/` in this slice but is **not yet wired into the scaffolder's ship list** (`skills/ingest/SKILL.md` Phase 1 Step 2 still ships six files). Wiring the seventh file in — including the `git add` line in Step 3 (the file is committed, not gitignored — the GM's voice belongs in version control), the `EXPECTED_SCAFFOLDED_FILES` and `EXPECTED_COMMITTED_FILES` tuples in `tests/test_ingest_scaffolding.py`, and the equivalent landing point in the shared scaffolder reference per [ADR-0020](./0020-modularization-via-shared-references.md) — is a separate follow-up slice. This ADR commits to the design (file, location, contract, glob set) so the follow-up is purely mechanical.
- Future authoring skills (`/init-campaign`, `/init-adventure`, future `/draft-*` skills) automatically pick up the style steering on matching paths with no per-skill opt-in, once the scaffolder wiring lands. New content directories added in future versions need their globs added to the stub (or the GM extends the campaign's own copy).
