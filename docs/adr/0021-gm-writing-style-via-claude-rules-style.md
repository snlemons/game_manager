...
## Amendment: mechanism-layer workaround for Claude Code #23478 (issue #121)

The path-scoped auto-load this ADR specifies is reliable on Read and Edit (Edit calls Read internally, so the rule auto-loads via the Read pathway). It is **not** reliable on the Write tool: per Claude Code [#23478](https://github.com/anthropics/claude-code/issues/23478), path-scoped rules silently fail to fire on Write. Drafting skills that create new artifacts via Write — `/wrap-session` Logs, `/prep-session` Briefs, `/ingest` per-doc Reference notes, and ad-hoc GM "create a new NPC at this path" interactions — bypassed `style.md` entirely. Issue [#119](https://github.com/snlemons/game_manager/issues/119) (2026-06-02 comment) carries the empirical diagnosis; PR [#120](https://github.com/snlemons/game_manager/pull/120) extended the `paths:` glob set to `.ttrpg-staging/**/*.md` so the auto-load fires during the staging-pattern UPDATE flow (`cp` + Edit), but the underlying Write-tool gap remained.

Issue [#121](https://github.com/snlemons/game_manager/issues/121) ships the mechanism-layer workaround: a **`CLAUDE.md` directive + PreToolUse reminder hook**. The directive tells the agent to Read `.claude/rules/style.md` once early in any drafting session before writing under the content-bearing paths; one Read amortizes across all subsequent drafts. The hook (`.claude/hooks/style-aware-write-gate.sh`, registered in `.claude/settings.json` as a `PreToolUse` matcher on `Write|Edit|MultiEdit`) emits a short reminder (~80 tokens, no style.md content) on every covered-path tool call as a per-call backstop confirming the path is in scope. Together they cover staging CREATEs, staging UPDATEs, direct covered-path CREATEs, direct covered-path Edits, and ad-hoc GM Writes.

The path-scoped `paths:` frontmatter from this ADR's core design remains the primary mechanism for Read/Edit; the CLAUDE.md directive is the primary mechanism for Write; the hook is the backstop for both. The `.ttrpg-staging/**/*.md` glob from PR #120 stays in place — it's harmless under the new approach and continues to help the staging-pattern UPDATE flow. If/when Anthropic resolves #23478, the hook becomes vestigial and the CLAUDE.md directive can soften from a hard "you must Read" to an informational pointer; the core design (file, location, contract, glob set) is unchanged.

Consequences specific to issue #121:

- New `templates/.claude/hooks/style-aware-write-gate.sh.template` — bash + jq script emitting a `hookSpecificOutput` reminder when the Write/Edit/MultiEdit target matches a content-bearing path. Committed to version control; scaffolder marks it executable.
- `templates/CLAUDE.md.template` "Writing style and voice" section: replaces the pre-#121 incorrect *"you don't need to ask the agent to consult it"* prose with the explicit Read directive, cites #23478, and extends the path list to include `.ttrpg-staging/`.
- `templates/.claude/settings.json.template` gains a top-level `hooks.PreToolUse` block registering the hook on `Write|Edit|MultiEdit`.
- `references/scaffolder.md` and `tests/_helpers.py` extend the file-enumeration set from seven rows to eight; the scaffolder marks the hook executable at write time.
- New `tests/test_style_hook.py` smoke-tests the hook's stdout contract (passthrough on empty path / missing style.md / non-matching path; reminder on each covered directory plus `.ttrpg-staging/`).
