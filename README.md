...
Start a fresh campaign repo via `/init-campaign` (post-v0.3 entry for net-new campaigns; see [ADR-0019](./docs/adr/0019-init-campaign-as-bootstrapping-front-door.md)). `/ingest` is the **extend-an-existing-campaign** entry — it requires an already-scaffolded campaign repo and hard-stops if invoked against an unscaffolded directory, directing you to `/init-campaign`.

To extend an existing scaffolded campaign with source docs:

1. Make sure the campaign repo is already scaffolded (markers: `CLAUDE.md`, `.claude/rules/sessions.md`, `.claude/rules/adventures.md`, `campaign.md`). If it isn't, run `/init-campaign` first.
...
The scaffolder (invoked from `/init-campaign`) writes:
...
It then runs `git init` and commits those files as the campaign repo's first commit. After that, the repo is yours — make your own commits, push it wherever you like.
