...
2. **Compute revisions from the GM's reply.** The agent decides what (if any) changes to the staged file the GM's reply implies — adding a section, appending a bullet, rephrasing a header, dropping a paragraph. Skill-specific question categories (e.g., `~/.claude/skills/ttrpg-gm/references/prep-session-questions.md` for `/prep-session`) define what kinds of revisions each kind of GM reply produces.
