...
The corresponding ADR is [ADR-0014](../docs/adr/0014-secrets-as-multi-container-lifecycle-objects.md) (introduced `kind` alongside `linked_secrets` to support Clue-vs-Beat queries). The full Beat schema, including the open-enum nature of `kind:`, lives in [`frontmatter-schemas.md`](./frontmatter-schemas.md) ("Beat" section). The Secret extraction heuristic that pairs with `kind: clue` lives in [`secret-extraction.md`](./secret-extraction.md).
...
Per `frontmatter-schemas.md` and CONTEXT.md, the starter values for `kind:` are:
...
- **At extraction time**, if the Beat's description names a Secret the campaign already has (via `secret-store.md`'s `find_dedup_candidates` or `find_by_container`), draft `linked_secrets:` with that Secret's slug.
- **If the Beat's description names a new Secret** (e.g., the scratchpad says "Clue: drop that Maren is the spy" and there's no `secrets/maren-is-the-spy.md` yet), the extracting skill should also propose the Secret per `secret-extraction.md`. The Beat and the Secret are co-proposed at the same staging review; the GM approves both together.
...
1. Extract the Secret from "Secrets and Lies" per `secret-extraction.md` → `secrets/mayor-funds-cult.md` (or analogous slug).
